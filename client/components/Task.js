...
function CheckMark({id, completed, toggleTodo}) {
  async function toggle() {
    const response = await fetch(`http://192.168.0.119:8080/todos/${id}`, {
      method: "PUT",
      headers: {
...
export default function Task({
  ...
  async function deleteTodo() {
    const response = await fetch(`http://192.168.0.91:8080/todos/${id}`, {
      method: "DELETE",
    });
    clearTodo(id);
    ...
    return (
        ...
        >
            ...
            {isDeleteActive && (
              <Pressable onPress={deteTodo} style={styles.deleteButton}>
